...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def __init__(self, defaultFilename=None, defaultPrefdefClassFile=None):
        ...
        loadBoxes = action('&Load Boxes', self.loadBoxes, 'v', 'koko', u'Load boxes from previous file')
        ...
        self.actions.beginner = (
            open, opendir, openNextImg, openPrevImg, loadBoxes, verify, save, None, create, copy, delete, None,
...
#            Default : select last item if there is at least one item
    ...
    def loadBoxes(self, _value=False):
#        if not self.mayContinue():
#            return
        ...
        if currIndex - 1 >= 0:
            ...
            if filename:
                ...
                if self.usingPascalVocFormat is True:
                    if self.defaultSaveDir is not None:
                        basename = os.path.basename(os.path.splitext(unicodeFilePath)[0]) + XML_EXT
                        xmlPath = os.path.join(self.defaultSaveDir, basename)
                        self.appendPascalXMLByFilename(xmlPath)
                    else:
                        xmlPath = unicodeFilePath.split(".")[0] + XML_EXT
                        if os.path.isfile(xmlPath):
                            self.appendPascalXMLByFilename(xmlPath)
                self.saveFile()
    ...
    #BEGIN KOKO_FUNC
    def appendPascalXMLByFilename(self, xmlPath):
        if self.filePath is None:
            return
        if os.path.isfile(xmlPath) is False:
            return
